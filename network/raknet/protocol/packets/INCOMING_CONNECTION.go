...
func NewNewIncomingConnection() *NewIncomingConnection {
	return &NewIncomingConnection{NewPacket(INCOMING_CONNECTION), "", 0, 0, 0, []string{"127.0.0.1"}, []uint16{0}, []byte{4}}
}
...
func (request *NewIncomingConnection) Encode() {
	...
	for i := 0; i < 20; i++ {
		if i < len(request.SystemAddresses) {	request.PutAddress(request.SystemAddresses[i], request.SystemPorts[i], request.SystemIdVersions[i])
		} else {
			request.PutAddress("0.0.0.0", 0, 4)
		}
	}
	request.PutUnsignedLong(request.PingSendTime)	request.PutUnsignedLong(request.PongSendTime)
}
