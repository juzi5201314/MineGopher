...
func NewServer(serverPath string, config *utils.Config, logger *utils.Logger) *Server {
	server = Server{}
...
func (server *Server) Start() {
	...
	server.logger.Info("MineGOpher "+ ServerVersion +", running on ")
	server.isRunning = true

	server.network = network.NewNetWork(server)
...
func (server *Server) mkdirs(path string) {
	...
	os.Mkdir(server.worldPath, 0700)
}
